...
| `LAWPDF_LM2_FASTTAB` | Off; `1`, `true`, `yes`, or `on` enables it | Loads the packaged FastTab ONNX line model ahead of the CatBoost default while retaining CatBoost as an inference fallback. | **keep** — experimental architecture switch; the 32-document gate blocked making it the default because source recall regressed on eight articles. |
...
| `LAWPDF_LM2_CONTEXT_ARBITER_MODEL` | First verified packaged residual-rescue model, when native CatBoost is active | Prepends an explicit context residual model path. The promoted rescue-only policy can change only non-keep actions to keep; it is intentionally bypassed by FastTab because it is authenticated to the CatBoost and baseline-context hashes. | **keep** — deployment/model-validation override; production packages verify the promoted asset with `--require-arbiter`. |
