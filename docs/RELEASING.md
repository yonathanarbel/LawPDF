...
2. Build both platform packages from the same commit. The Windows job installs the exact installer, compares its hash and the installed executable against the packaged executable, checks product/file/runtime versions and every LawPDF Start Menu shortcut, and requires successful native/context/model runtime status. The installed runtime runs from its installation folder; each reported model/library path must point to an existing file inside that folder so a source checkout cannot hide missing packaged assets. Retain the installation evidence with the release. A hosted runner is not evidence of testing on a user's own Windows hardware.
